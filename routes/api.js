...
const database = require('../models/index') //will require the index.js file from the directory
...
router.get('/students', function(req, res, next){
    ...
    Student.findAll( { ord: ['present','name']}).then (students => {
...
router.patch('/students/:id', function(req, res, next){
    ...
    Student.update(updatedStudent, { where: {id: studentID} }).then(( result )=> {
        ...
        //if 1 row was changed, we found student and thery were updated
        if (rowsModified === 1) {
            res.send('OK')
        }
        //student id that doesn't exists
        else {
       ...
       if ( err instanceof database.Sequelize.ValidationError) {
        ...
        return res.status(400).json(messages)
           ...
           // some other error? - can't connect
           next (err) 
      }
    })
    // that kinds of errors could we see?
    // student id that doesn't exists
    // invalid data in the updatedStudent - eg.no name
